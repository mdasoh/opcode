...
{
    bfd_vma    vma = 0x100000;//= bfd_get_start_address(abfd);
    int passes[ 2 ];
    pipe( passes );
...
{
   unsigned int ulimit = 0x100000 + SIZE;
   int bah = 0;
...
int main( int argc, char* argv[] )
{
   ...
   else
   {
      SIZE = szfile.st_size;
      code = malloc( SIZE );
      FILE* boom = fopen( argv[ 1 ], "r" );
      fread( code, SIZE, 1, boom );
      fclose( boom );
   }
   ...
   abfd -> start_address = 0x100000; // 1 MiB.
